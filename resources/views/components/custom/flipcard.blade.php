<div>
    <div class="hidden lg:block group relative h-96 w-72 [perspective:1000px]">
        <div
            ...
            <div class="absolute w-full h-full rounded-xl p-6 text-white [backface-visibility:hidden]"
                style="background-image: url('{{ $backgroundImageFront }}'), linear-gradient(to bottom, rgba(128, 128, 128, 1), rgba(0, 0, 0, 1));background-size: contain; background-position: center; background-repeat: no-repeat;">
    ...
    <!--Mobile-->
    <div
        class="block lg:hidden relative w-full h-40 bg-gradient-to-r from-gray-700 to-gray-900 rounded-xl shadow-lg text-white p-6">
        <div class="flex flex-col justify-between items-center h-full">
            <div class="text-xl font-semibold">
                Divisi {{ $title ?? 'Card Title' }}
            </div>

            <!-- Tombol Lihat Karya Kami -->
            <a href="{{ route($actionRoute) }}"
                class="px-4 py-2 bg-white text-red-600 rounded-lg font-semibold hover:bg-opacity-90 transition-colors">
                {{ $actionText }}
            </a>
