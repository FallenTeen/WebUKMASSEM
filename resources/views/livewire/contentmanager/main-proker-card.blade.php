<div class="flex flex-wrap gap-4 justify-center {{ $containerClass }}">
    @foreach ($prokers as $proker)
        @if ($proker->url)
            <!-- Make it clickable if there's a URL -->
        ...
        @else
            <!-- Just show content without link if there's no URL -->
            <div class="relative w-full sm:w-48 lg:w-96 overflow-hidden rounded-2xl px-4 sm:px-8 pb-8 pt-40 mt-12 transition-transform transform hover:-translate-y-4 duration-400">
        @endif
            ...
            <!-- Title (should always be visible and not hidden by overlay) -->
            ...
            <!-- Description (this is shown after hovering) -->
        ...
        @if ($proker->url)
            </a> <!-- Close the <a> tag if there's a URL -->
        @else
            </div> <!-- Close the <div> tag if there's no URL -->
        @endif
    @endforeach
</div>
