            ...
            <div class="flex items-center space-x-3 w-full md:w-auto">
                <div class="relative flex-1 md:flex-none">
                    <x-dropdown align="right" width="48">
                        <x-slot name="trigger">
                            <button
                                class="flex items-center justify-center w-full md:w-40 px-4 py-2 text-sm font-medium text-gray-700 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-300 dark:border-gray-700 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-700 transition">
                        ...
                        <x-slot name="content" class="py-1">
                            <x-dropdown-link wire:click="toggleFilter('published')" class="flex items-center">
                                <span class="w-2 h-2 mr-2 bg-blue-500 rounded-full"></span>
                                {{ __('Published') }}
                            </x-dropdown-link>
                            <x-dropdown-link wire:click="toggleFilter('draft')" class="flex items-center">
                ...
                <a href="{{ route('admin.addproker') }}"
                    class="flex items-center justify-center w-full md:w-auto px-4 py-2 text-sm font-medium text-white bg-merah border border-transparent rounded-lg hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 transition">
                    <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="space-y-6">
            <div class="bg-white dark:bg-gray-800 shadow-sm rounded-lg p-4 space-y-4">
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div class="relative">
                        <input type="text" wire:model.live.debounce.300ms="search" placeholder="Search prokers..."
                        ...
                        <svg class="absolute left-3 top-3 w-4 h-4 text-gray-400 dark:text-gray-500" fill="none"
                ...
                @if (session()->has('message'))
                    <div class="p-4 bg-green-50 dark:bg-green-900 border-l-4 border-green-400 dark:border-green-600">
                        <div class="flex items-center">
                            <svg class="w-5 h-5 text-green-400 dark:text-green-300 mr-3" fill="none" stroke="currentColor"
                                viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                ...
                                <tr class="hover:bg-gray-50 dark:hover:bg-gray-900 transition">
                                    ...
                                    <td class="px-6 py-4">
                                        <div class="text-sm font-medium text-gray-900 dark:text-white">
                                            {{ $proker->judul }} {{ $proker->tanggal->format('Y') }}
                                        </div>
                                    </td>
                                    <td class="px-6 py-4 text-sm text-gray-500 dark:text-gray-400">
                                        <div class="relative">
                                            ...
                                            <div class="prose prose-sm max-w-none">
                                                {!! Str::limit(strip_tags($proker->deskripsi), 100) !!}
                                                @if(strlen(strip_tags($proker->deskripsi)) > 100)
                                                    <button type="button" x-data="{}"
                                                        x-on:click="$dispatch('open-modal', { id: 'view-description-{{ $proker->id }}' })"
                                                        class="text-merah hover:text-red-700 font-medium text-xs ml-1 focus:outline-none transition duration-300">
                                                        Baca Selengkapnya
                                                    </button>
                                                @endif
                                            </div>
                                        ...
                                        <div x-data="{ open: false }" x-show="open"
                                            x-on:open-modal.window="$event.detail.id === 'view-description-{{ $proker->id }}' ? open = true : null"
                                            ...
                                            <!-- Modal backdrop -->
                                            <div class="fixed inset-0 bg-black bg-opacity-50" x-on:click="open = false">
                                            </div>
                                            ...
                                            <div class="relative flex items-center justify-center min-h-screen p-4">
                                                <div
                                                    ...
                                                    <div
                                                        class="px-6 py-3 bg-gray-50 dark:bg-gray-700 flex justify-end border-t dark:border-gray-600">
                                                        <button type="button" x-on:click="open = false"
                                                            class="px-4 py-2 bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 rounded-md hover:bg-gray-300 dark:hover:bg-gray-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-gray-500 transition-colors duration-300">
                                                            Tutup
                                                        </button>
                                                    </div>
                                                </div>
                                    ...
                                    <td class="px-6 py-4">
                                        <span class="px-2.5 py-0.5 text-xs font-medium rounded-full 
                                            @if($proker->kategori === 'primer') bg-blue-100 text-blue-800
                                            @elseif($proker->kategori === 'sekunder') bg-green-100 text-green-800
